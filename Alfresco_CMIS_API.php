...
*/	
		...
		if($rel=='alternate' && $title=='doclib')$this->thumbnailUrl=$href;
	...
	//add new structure of aspectcmis->e1->cmis 
	else if(isset($this->namespaces['e1'])){
		$e1data=$cmis->children($this->namespaces['e1']);
		//fix extract aspect issue from cmis->aspect to cmis->e1->cmis
		$aspectsdata=$e1data->aspects->properties->children($this->namespaces['cmis']);
		//echo("ccccccccccccc");
		//print_r($aspectsdata);
		//echo("ccccccccccccc");
		//$aspectsdata=$cmis->children($this->namespaces['e1']);

		//Getting object PROPERTIES within different attributes
		//PropertyString
		for($x=0;$x<count($aspectsdata->propertyString);$x++){
			$propertyDefinitionId=$aspectsdata->propertyString[$x]->attributes()->propertyDefinitionId;
			$value=(string)$aspectsdata->propertyString[$x]->value;
			if($value)$this->aspects["$propertyDefinitionId"]=$value;
		}
		//PropertyId
		for($x=0;$x<count($aspectsdata->propertyId);$x++){
			$propertyDefinitionId=$aspectsdata->propertyId[$x]->attributes()->propertyDefinitionId;
			$value=(string)$aspectsdata->propertyId[$x]->value;
			if($value)$this->aspects["$propertyDefinitionId"]=$value;
		}
